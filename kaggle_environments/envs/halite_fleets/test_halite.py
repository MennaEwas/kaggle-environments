...
def test_halite_helpers():
    ...
    def helper_agent(board):
        for ship in board.current_player.ships:
            ship.next_action = FleetAction.NORTH
        for shipyard in board.current_player.shipyards:
            shipyard.next_action = ShipyardActionType.SPAWN
...
def test_move_moves_ship():
    ...
    for ship in board.current_player.ships:
        ship.next_action = FleetAction.SOUTH
    next_board = board.next()
    for ship in board.fleets.values():
...
def move_toward(ship, target: Point):
    ...
    (x2, y2) = target
    if x2 > x1:
        return FleetAction.EAST
    elif x2 < x1:
        return FleetAction.WEST
    elif y2 > y1:
        return FleetAction.NORTH
    elif y2 < y1:
        return FleetAction.SOUTH
...
def test_ship_shipyard_collision_destroys_both():
    ...
    opponent_ship.next_action = FleetAction.CONVERT
    board = board.next()
    assert len(board.fleets) == 1
    assert len(board.shipyards) == 1
    while player_ship.id in board.fleets:
        board.fleets[player_ship.id].next_action = move_toward(player_ship, opponent_ship.position)
        board = board.next()
    assert len(board.fleets) == 0
...
def test_no_move_on_halite_gathers_halite():
    board = create_board(starting_halite=1000, agent_count=1)
    ship = first(board.fleets.values())
...
def test_failed_convert_gathers_halite():
    ...
    ship = first(board.fleets.values())
    ship.next_action = FleetAction.CONVERT
    expected_delta = int(ship.cell.halite * board.configuration.collect_rate)
    next_board = board.next()
    next_ship = next_board.ships[ship.id]
    ship_delta = next_ship.halite - ship.halite
